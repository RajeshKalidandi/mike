"""Configuration settings for Mike.
...
class DatabaseConfig(BaseModel):
    ...
    path: Path = Field(
        default_factory=lambda: Path.home() / ".mike" / "mike.db",
...
class PathsConfig(BaseModel):
    ...
    cache_dir: Path = Field(
        default_factory=lambda: Path.home() / ".mike" / "cache",
        description="Cache directory path",
    )
    temp_dir: Path = Field(
        default_factory=lambda: Path(Path(os.getenv("TMPDIR", "/tmp"))) / "mike",
        description="Temporary directory path",
    )
    output_dir: Path = Field(
        default_factory=lambda: Path.cwd() / "mike_output",
        description="Default output directory",
    )
    vector_store_dir: Path = Field(
        default_factory=lambda: Path.home() / ".mike" / "vector_store",
        description="Vector store directory",
    )
    sessions_dir: Path = Field(
        default_factory=lambda: Path.home() / ".mike" / "sessions",
        description="Sessions data directory",
    )
    config_dir: Path = Field(
        default_factory=lambda: Path.home() / ".mike",
...
class Settings(BaseModel):
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            ...
            "description": "Complete configuration for the Mike system",
        },
    )
