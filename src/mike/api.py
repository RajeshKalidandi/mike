"""Public API interface for Mike.

This module provides the main entry point for using Mike programmatically.
...
from mike.orchestrator.state import (
    AgentType,
    ExecutionMode,
    ExecutionStatus,
)
from mike.orchestrator.engine import AgentOrchestrator, Agent, AgentRegistry
...
class Mike:
    """Main entry point for Mike.

    This class provides a unified, high-level interface for all Mike
    ...
    def __init__(
        ...
        logger.info(f"Mike initialized (db: {self.settings.database.path})")
        ...
        for progress in generator:
            ...
            if progress.status in ["completed", "failed"]:
                ...
                if progress.status == "failed" and progress.error:
                    raise MikeError(progress.error)
        ...
        try:
            if is_git_url:
                ...
                if clone_path is None:
                    raise MikeError(f"Failed to clone repository: {path_str}")
                scan_path = clone_path
                source_path = path_str
            else:
                scan_path = os.path.abspath(path_str)
                if not os.path.exists(scan_path):
                    raise MikeError(f"Path not found: {path_str}")
    ...
    def analyze(
        ...
        if session is None:
            raise MikeError(f"Session not found: {session_id}")
    ...
    def generate_docs(
        ...
        Raises:
            MikeError: If documentation generation fails
    ...
    def rebuild_project(
        ...
        Raises:
            MikeError: If scaffolding fails
...
class MikeError(Exception):
    """Exception raised for Mike errors."""
...
def create_ai(
    ...
    """Create an Mike instance with sensible defaults.

    This is a convenience function that creates an Mike instance
