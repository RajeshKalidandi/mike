# Mike Monitoring Module

A comprehensive monitoring and telemetry system for tracking agent performance, system metrics, and usage statistics in Mike.
...
```python
from mike.monitoring import TelemetryCollector, MetricsRegistry
...
mike telemetry stats --session <session_id>

# Output in different formats
mike telemetry stats --format json
mike telemetry stats --format markdown
```
...
mike telemetry report --format json

# Generate Markdown report and save to file
mike telemetry report --format markdown --output report.md

# Report for specific session
mike telemetry report --session <session_id>
```

### Generate Dashboard
```bash
# Generate HTML dashboard
mike telemetry dashboard --output dashboard.html

# Generate and serve on localhost
mike telemetry dashboard --serve --port 8080
```

### View Metrics
```bash
# Show current metrics
mike telemetry metrics

# Export in Prometheus format
mike telemetry metrics --format prometheus

# Export as JSON
mike telemetry metrics --format json
```
