"""Main Streamlit application for Mike.
...
from mike.scanner.clone import clone_repository, is_git_url
from mike.scanner.scanner import FileScanner
from mike.web.components import (
    render_agent_result,
...
st.set_page_config(
    ...
    menu_items={
        "Get Help": None,
        "Report a bug": None,
        "About": "Mike - Local AI Software Architect",
    },
)
...
def render_home():
    ...
    st.title("🏠 Welcome to Mike")

    st.markdown(
        """
        ### Local AI Software Architect
        
        Mike is a fully local, offline-capable AI system that ingests any codebase
        and produces:
            ...
            if st.button("📦 Extract and Create Session", use_container_width=True):
                with st.spinner("Extracting and processing..."):
                    try:
                        ...
                        from mike.web.utils import calculate_content_hash
            ...
            if not is_git_url(git_url):
                ...
                with st.spinner("Cloning repository..."):
                    try:
                        ...
                        temp_dir = Path(tempfile.gettempdir()) / "mike_repos"
                        temp_dir.mkdir(parents=True, exist_ok=True)
...
def render_visualizations():
    ...
    try:
        ...
        if "selected_file" in st.session_state and st.session_state.selected_file:
            ...
            from mike.web.components import render_code_viewer
...
def render_settings():
    ...
    db_path = st.text_input(
        "Database Path",
        value=settings.get("db_path", "~/.mike/mike.db"),
    )

    log_dir = st.text_input(
        "Log Directory",
        value=settings.get("log_dir", "~/.mike/logs"),
    )

    output_dir = st.text_input(
        "Output Directory",
        value=settings.get("output_dir", "~/.mike/output"),
    )
    ...
    with col2:
        if st.button("🔄 Reset to Defaults", use_container_width=True):
            from mike.web.utils import DEFAULT_SETTINGS
