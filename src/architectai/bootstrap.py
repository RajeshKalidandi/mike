"""System initialization and bootstrap utilities for ArchitectAI.

This module handles first-time setup, dependency checking, and system
initialization to ensure ArchitectAI is ready to use.
"""
...
def setup_directories(settings: Optional[Settings] = None) -> Dict[str, Path]:
    """Create all required directories for ArchitectAI.

    Creates the following directories if they don't exist:
    - ~/.architectai/ (config)
    - ~/.architectai/cache/
    - ~/.architectai/vector_store/
    - ~/.architectai/sessions/
    - ~/.architectai/sessions/logs/
...
def bootstrap(
    ...
    logger.info("=" * 60)
    logger.info("ArchitectAI Bootstrap")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    if result["success"]:
        logger.info("✓ Bootstrap complete! ArchitectAI is ready.")
    else:
...
def reset_system(
    ...
    """Reset ArchitectAI to factory defaults.
    ...
    if not force:
        response = input(
            "Are you sure you want to reset ArchitectAI? This will delete all data! [y/N]: "
        )
        if response.lower() != "y":
