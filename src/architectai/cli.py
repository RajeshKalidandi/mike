...
from architectai.config.commands import get_config_group
from architectai.monitoring.telemetry import TelemetryCollector
from architectai.monitoring.metrics import MetricsRegistry
...
def status(ctx: click.Context) -> None:
    ...
    if output_format == "json":
        ...
        click.echo(f"ArchitectAI v{system_status.get('version', 'unknown')}")
        click.echo(f"Database: {system_status.get('database_path', 'unknown')}")
        click.echo(f"Sessions: {system_status.get('session_count', 0)}")
        click.echo(f"\nAgents:")
        for name, info in system_status.get("agents", {}).items():
...
@main.command()
@click.argument("session_id")
@click.option("--db-path", default="architectai.db", help="Path to database")
...
def search(
    ...
    try:
        ...
        if session_id not in vector_store.list_sessions():
            ...
            click.echo("Run 'architectai embed' first")
            return
