"""Configuration CLI commands for ArchitectAI."""
...
def get_config_group() -> click.Group:
    ...
    @click.option(
        ...
        "global_",
        is_flag=True,
        help="Create global config in ~/.architectai/",
    )
    @click.option(
        "--local",
        "local_",
        is_flag=True,
        help="Create local config in ./.architectai/",
    )
    @click.option(
        "--force",
        "-f",
    ...
    def _print_config_table(settings: Settings) -> None:
        ...
        click.echo()
        click.echo("=" * 60)
        click.echo("ArchitectAI Configuration")
        click.echo("=" * 60)
    ...
    def config_profiles(ctx: click.Context) -> None:
        ...
        click.echo("\n" + "=" * 60)
        click.echo("\nUse: architectai config use-profile <name>")
    ...
    def config_schema(ctx: click.Context, output: Optional[str]) -> None:
        ...
        from architectai.config.settings import generate_schema
